...
socio_dem |>
  ...
  mutate(term = case_when(var == "work_pwk" & term == "No (REF)" ~ "Did not work", 
                          ...
                          var == "l_indigena" & term == "No (REF)" ~ "Doesn't speak indigenous language", 
         ...
         color = factor(c(rep(1, 2), rep(2, 4), rep(3, 3), rep(4, 2), rep(5, 4), 
                   ...
                   labels = c("Sex", "Region", "Age group", "Urbanicity", "Education", 
                              "SES", "Insurance", "Indigenous", "Past-week work"))) |>
  ...
  labs(y = "", 
       title = "Figure 1. Sociodemographic predictor of Severe Visual Impairment among the Mexican population.")


# ggsave("forestplot.pdf", width = 12, height = 6, device = "pdf", units = "in", dpi = 1800)
# ggsave(here("results/", "forestplot.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)
...
chr_dis |>
  bind_rows() |>
  mutate(term = case_when(var == "obesidad" & term == "No (REF)" ~ "No obesity", 
                          var == "obesidad" & term == "Yes" ~ "Obesity", 
                          var == "diab_dv" & term == "No (REF)" ~ "No VI due to diabetes", 
                          var == "diab_dv" & term == "Yes" ~ "VI due to diabetes",
                          var == "diab_perd_vsn" & term == "No (REF)" ~ "No vision loss due to diabetes", 
                          var == "diab_perd_vsn" & term == "Yes" ~ "Vision loss due to diabetes",
                          var == "hta_retina" & term == "No (REF)" ~ "No retinal damage due to HBP", 
                          var == "hta_retina" & term == "Yes" ~ "Retinal damage due to HBP",
                          var == "cardio_dis" & term == "No CVD (REF)" ~ "No CVD", 
                          var == "cardio_dis" & term == "Yes" ~ "CVD",
                          var == "kidney_dis" & term == "No (REF)" ~ "No kidney disease", 
                          var == "kidney_dis" & term == "Yes" ~ "Kidney disease",
                          var == "dyslipidemia" & term == "No (REF)" ~ "No dyslipidemia", 
         ...
         color = factor(c(rep(1, 2), rep(2, 4), rep(3, 2), rep(4, 2), rep(5, 2), rep(6, 2), 
                   ...
                   labels = c("Obesity", "Diabetes and HTN", "VI due to diabetes", 
                              "Vission loss due to diabetes", "Retinal damage due to HBP", 
  ...
  theme(legend.position = "right", 
        ...
        text = element_text(size = 12)) +
  labs(y = "", 
       title = "Figure 2. Health predictors of Severe Visual Impairment among the Mexican population.")


# ggsave("forestplot.pdf", width = 12, height = 6, device = "pdf", units = "in", dpi = 1800)
# ggsave(here("results/", "forestplot2.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)

```

```{r}
var_mh <- names(ensanut18_4analysis)[c(33, 38:40, 43)]

mh_res <- foreach(i = var_other) %dopar% {
  reg_fun(i, "disc_visual_severa_num")
...
mh_res |>
  ...
  mutate(term = case_when(var == "cesd" & term == "No depressive symptoms (REF)" ~ "No depressive symptoms (REF)", 
                          ...
                          var == "alcohol" & term == "No" ~ "Former alcohol usel",
  ...
  theme(legend.position = "right", 
        ...
        text = element_text(size = 12)) +
  labs(y = "", 
       title = "Figure 3. Mental Health Predictors of Severe Visual Impairment among the Mexican population.")

ggsave(here("results/", "forestplot3.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)
...
accidente_res |>
  ...
  mutate(term = case_when(var == "accidente" & term == "No (REF)" ~ "No accident in the past year (REF)", 
                          ...
                          var == "dis_accidente" & term == "Limitación para hablar u oír" ~ "Speaking/hearing disability", 
  ...
  theme(legend.position = "right", 
        ...
        text = element_text(size = 12)) +
  labs(y = "", 
       title = "Figure 4. Disability as Predictor of Severe Visual Impairment among the Mexican population.") 

ggsave(here("results/", "forestplot4.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)
