...
```{r}
var_chr_dis <- names(ensanut18_4analysis)[c(34, 44, 35:37)]
...
# save(socio_dem, chr_dis, file = here("data/", "age_cat_adjusted_estimates.RData"))
...
# write_csv(socio_dem_coefs, here("results/", "socio_dem_coefs_age_cat_adj.csv"))
...
socio_dem_coefs |>
  ...
  scale_x_continuous(limits = c(0.2, 8.00), breaks = c(0.7, seq(1.0, 8.0, 1.0)), name = "Prevalence Ratio (95% CI)")  +
...
# ggsave(here("results/", "forestplot_age_cat_adj.pdf"), width = 12, height = 6, device = "pdf", units = "in", dpi = 1800)
# ggsave(here("results/", "forestplot_age_cat_adj.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)
...
# write_csv(chr_dis_coef, here("results/", "chr_dis_coef_age_cat_adj.csv"))
...
chr_dis_coef |>
  ...
  mutate(term = case_when(var == "obesidad" & term == "No (REF)" ~ "No obesity (REF)", 
         ...
         term =  factor(term),  
         orden = seq(1, length(var), 1),
         color = factor(c(rep(1, 2), rep(2, 4), rep(3, 2), 
                   rep(4, 2), rep(5, 2)), 
                   labels = c("Obesity", "Diabetes and HTN",
  ...
  geom_point(size = 5, shape = "diamond") +
  geom_errorbarh(height = 0.3) + 
  scale_x_continuous(limits = c(0.01, 8.00), breaks = c(0.7, seq(1.0, 8.0, 1.0)), name = "Prevalence Ratio (95% CI)")  +
  geom_vline(xintercept = 1, color = "black", linetype = "solid", alpha = 0.5) +
  ggthemes::theme_calc() +
  theme(legend.position = "left", 
        legend.title = element_blank(), 
...
ggsave(here("results/", "forestplot2_age_cat_adj_v2.pdf"), width = 12, height = 6, device = "pdf", units = "in", dpi = 1800)
ggsave(here("results/", "forestplot2_age_cat_adj_v2.jpeg"), width = 12, height = 6, units = "in", dpi = 1800)
