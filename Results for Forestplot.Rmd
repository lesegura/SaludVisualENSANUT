...
data <- read_sav("2022Ensanut2018.sav")
data.csv <- read_csv("CS_ADULTOS.csv")
...
results <- ref_levels[1, ] %>%
  ...
  bind_rows(mylist[3:4, ]) %>%
  bind_rows(ref_levels[4, ]) %>%
  bind_rows(mylist[5:7, ]) %>%
  bind_rows(ref_levels[5, ]) %>%
  bind_rows(mylist[8:10, ]) %>%
